...
function onHavingLocation(position) {
  ...
  query_header = 'https://moto.data.socrata.com/resource/wrmr-tdyp.json?$order=incident_datetime desc&$where=';
  ...
  $.get(query).done(function(search_results) {
    ...
    for (const e of search_results) {
      ...
      year_distribution.push(e.incident_datetime.slice(0, 5));
      ...
      text += 'distance: ' + distance + ' miles<br/>';
      text += '<hr/>';
    }
