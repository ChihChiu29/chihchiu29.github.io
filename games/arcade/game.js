    ...
    updateHighScores(congrats) {
        FIREBASE.readHighScores().then((highScores) => {
            ...
            for (const record of GLOBAL.bestScores) {
                scoreTexts.push(`${idx + 1}. ${record.fromUser} ${record.score.toFixed(1)}`);
                idx++;
            }
            const rotatingText = new RotatingText(this, congrats.x - 150, congrats.y + 100, 300, 150);
